...
function getStatePath(name)
	return StateDirectory .. "\\" .. name .. ".state"
end
...
function storeFailure(type)
	...
	if fileExists(statePath) == false then
		if type == "timedOut" then
			table.insert(StatePool, stateName)
		end
		saveState(stateName)
	end
end
...
saveLoadFile = forms.textbox(form, StateFilename .. ".pool", 170, 25, nil, 5, 148)
...
while true do
	...
	if pool.currentFrame % 250 == 0 then
		storeWIP()
	end
	...
	getPositions()
	-- this may prevent mario from going left
